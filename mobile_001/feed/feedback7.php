...
function metlabel_feedback7($fid,$mobile){
		...
		foreach($fd_para as $key=>$val){
			...
			switch($val[type]){
				case 1:
					...
					$val[type_html]="<input name='para{$val[id]}' type='text' class='input-text {$wr_ok}' style='font-size:22px;'  placeholder='{$val[name]}' />";
				break;
				case 2:
    ...
    $center = $val_input[4];
   	array_pop($val_input);
    array_splice($val_input,1,0,$center);
   	foreach($val_input as $k=>$v)
   	{
   		$metinfo.=$v;
   	}
 	if($met_memberlogin_code==1){
				$metinfo.="\n";
				$metinfo.="<input name='code' class='qcode' type='text' size='6' maxlength='8' placeholder='验证码' >";
				$metinfo.="<img align='absbottom' class='qcodema' src='{$navurl}member/ajax.php?action=code'  onclick=this.src='{$navurl}member/ajax.php?action=code&'+Math.random() title='".$lang_memberTip1."'/>";
	}
     ...
     $metinfo.="<input type='submit' value='立即提交'>\n";
     $metinfo.="</form>\n";
	}
	return $metinfo;
}
