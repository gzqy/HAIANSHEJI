<?php
//反馈提交表单函数
function metlabel_feedback11($fid,$mobile){
    ...
    foreach($fd_para as $key=>$val){
 	...
 	if($met_memberlogin_code==1){
				$metinfo.="<tr>\n";
				$metinfo.="<td><input name='code' class='inp' type='text' size='6' maxlength='8' style='width:134px;height:35px' placeholder='验证码' >";
				$metinfo.="<img align='absbottom' id='now_code' src='{$navurl}member/ajax.php?action=code'  onclick=this.src='{$navurl}member/ajax.php?action=code&'+Math.random() title='".$lang_memberTip1."'/>";
				$metinfo.="</td>\n";
				$metinfo.="</tr>\n";
	}
     ...
     $metinfo.="<input id='btnone' type='submit' name='Submit' value='立即预约设计师'>\n";
     $metinfo.="</form>\n";
	}
	return $metinfo;
}
