...
function metlabel_feedback10($fid,$mobile){
	...
	break;
	case 3:
	$list[input]="<textarea name='para$list[id]'     class='form-text'  class='form-text' placeholder='{$list[description]}'></textarea>";
	break;
	case 4:
	$i=0;
	foreach($paravalue[$list[id]] as $key=>$val){
	...
	if($mobile){
		...
		$metinfo = $metinfo1;
		  ...
		  $metinfo =$fdjs;
			$array  = array(
		"<div class='form-1'><i class='icon'><img src='../templates/mobile_001/images/img/jiudian/me.png'></i>",

		"</div><div class='form-1'><i class='icon-1'><img src='../templates/mobile_001/images/img/jiudian/phone.png'></i>",
		"</div><div class='form-2'><div class='formst'>",

		"</div><div class='formst'>",
		"</div><i class='triangle'></i></div>",
		"</div>"
		
	);
	 $metinfo.="<form  class='form-main'  enctype='multipart/form-data' method='POST' id='form1' onSubmit='return Che();' name='myform' action='{$lujin}index.php?action=add&lang=".$lang."' target='_self'>\n";
   	foreach($fd_para as $key=>$val){
    ...
    $metinfo = $metinfo;
     ...
     $metinfo.="<button class='yvyuenow heartbeat' disabled  id='btn' >我要预约设计师</button>\n";
