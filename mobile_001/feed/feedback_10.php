<?php
//反馈提交表单函数
function metlabel_feedback10($fid,$mobile){
	...
	switch($list[type]){
	case 1:
	$list[input]="<input name='para$list[id]'  class='para$list[id]'  type='text' data-required='1'  onmouseleave='checkInput()' oninput='changeBtnable()' size='30' placeholder='{$list[description]}' />";
	break;
 	case 2:
	$list[input]="<select class='msgSel' name='para$list[id]'><option data-required='1' selected='selected' >{$lang_Choice}省份</option>";
	...
	break;
	case 3:
	$list[input]="<textarea name='para$list[id]'  data-required='1'  class='form-text' placeholder='{$list[description]}'></textarea>";
	break;
	case 4:
	$i=0;
	foreach($paravalue[$list[id]] as $key=>$val){
...
$fdjs="<script language='javascript'>";
	...
	if($mobile){
		...
		$metinfo = $metinfo1;
		  ...
		  $metinfo =$fdjs;
			$array  = array(
		"<div class='form-1'><i><img src="."../templates/mobile_001/images/img/me.png"."></i>","</div><div class='form-1'><i><img src='../templates/mobile_001/images/img/phone.png' ></i>    ","</div><div class='form-2'><div class='formst'>","</div><div class='formst'>","</div></div>"
	);
	 $metinfo.="<form  class='form-main ui-from' enctype='multipart/form-data' method='POST' id='form1' onSubmit='return Che();' name='myform' action='{$lujin}index.php?action=add&lang=".$lang."' target='_self'>\n";
   	foreach($fd_para as $key=>$val){
    ...
    $metinfo = $metinfo;
     ...
     $metinfo.="<button class='heartbeat ' disabled  id='btn' >立即预约设计师</button>\n";
