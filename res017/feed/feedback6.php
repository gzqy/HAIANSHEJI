<?php
//反馈提交表单函数
function metlabel_feedback6($fid,$mobile){
	...
	switch($list[type]){
	case 1:
	$list[input]="<input name='para$list[id]' type='text' size='30'  class='liu' placeholder='{$list[description]}' />";
	break;
	case 2:
	$list[input]="<div class='theareabox'><input name='para$list[id]' type='text' size='30'  class='liu' placeholder='{$list[description]}' /></div>";
	break;
	...
	break;
	case 5:
	$list[input]="<div class='plceadarea'><input name='para$list[id]' type='text' class='input' placeholder='{$list[description]}'></div>";
	break;
	case 6:
	$i=0;
	foreach($paravalue[$list[id]] as $key=>$val){
	...
	if($mobile){
     ...
     $metinfo.="<div class='formmainleft'>\n";
    foreach($fd_para as $key=>$val){
     $metinfo.=$val[input];
    }
    ...
    if($met_memberlogin_code==1){
				$metinfo.="<div class='codebox'>\n";
				$metinfo.="<tr>\n";
				$metinfo.="<td class='input'><input name='code' type='text' class='code' id='code' placeholder='验证码' size='6' maxlength='8'>";
				$metinfo.="<img align='absbottom' id='new_code' src='{$navurl}member/ajax.php?action=code'  onclick=this.src='{$navurl}member/ajax.php?action=code&'+Math.random() title='".$lang_memberTip1."'/>";
				$metinfo.="</td>\n";
				$metinfo.="</tr>\n";
				$metinfo.="</div>\n";
			}
     ...
     $metinfo.="<input id='btnnow' type='submit' name='Submit' value='立即预约设计师'>\n";
     $metinfo.="</form>\n";
	}
	return $metinfo;
}
