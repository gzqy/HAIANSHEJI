<?php
//反馈提交表单函数
function metlabel_feedback8($fid,$mobile){
	...
	if($mobile){
     ...
     $metinfo =$fdjs;
     $metinfo.="<form enctype='multipart/form-data' class='submissionform' method='POST' name='myform' onSubmit='return Checkfeedback();' action='{$lujin}index.php?action=add&lang=".$lang."' target='_self'>\n"; 
    ...
    if($met_memberlogin_code==1){ 
				$metinfo.="<input name='code' type='text' class='yanzhengmatxt focuschange' placeholder='验证码' size='6' maxlength='8'>";
				$metinfo.="<img align='absbottom' class='yanzhengma' src='{$navurl}member/ajax.php?action=code'  onclick=this.src='{$navurl}member/ajax.php?action=code&'+Math.random() title='".$lang_memberTip1."'/>";
			}
			 $metinfo.="</div>\n"; 
     ...
     $metinfo.="<input type='submit' name='Submit' value='提交需求'>\n";
     $metinfo.="</form>\n";
	}
	return $metinfo;
}
