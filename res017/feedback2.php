<?php
//反馈提交表单函数
function metlabel_feedback2($fid,$mobile){
	...
	switch($list[type]){
	case 1:
	$list[input]="<div class='footer_iptline'><input name='para$list[id]' type='text' size='30'  class='liu' onblur=".'"'."if(this.value==''){this.value='{$list[description]}';}".'"'."  onfocus=".'"'."if(this.value=='{$list[description]}'){this.value='';}".'"'." value='{$list[description]}' /></div>";
	break;
	case 2:
	...
	break;
	case 3:
	$list[input]="<textarea name='para$list[id]' class='fuyan' onblur=".'"'."if(this.placeholder==''){this.placeholder='{$placeholder[description]}';}".'"'."  onfocus=".'"'."if(this.placeholder=='{$list[description]}'){this.placeholder='';}".'"'." placeholder='{$list[description]}'></textarea>";
	break;
	case 4:
	$i=0;
	foreach($paravalue[$list[id]] as $key=>$val){
...
if($met_memberlogin_code==1){  
     $metinfo.=$lang_memberImgCode;
     $metinfo.="<input name='code' onKeyUp='pressCaptcha(this)' type='text' class='code' id='code' size='6' maxlength='8' style='width:50px' />";
     ...
     $metinfo.="<div class='footer_intbtn'><input type='submit' name='Submit' value='7分钟立即回电' class='fas'></div>\n";
     $metinfo.="</form>\n";
	}
	return $metinfo;
}
