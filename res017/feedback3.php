<?php
//反馈提交表单函数
function metlabel_feedback3($fid,$mobile){
	...
	switch($list[type]){
	case 1:
	$list[input]="<div class='iptline'><input name='para$list[id]' type='text' size='30'  class='liu' onblur=".'"'."if(this.value==''){this.value='{$list[description]}';}".'"'."  onfocus=".'"'."if(this.value=='{$list[description]}'){this.value='';}".'"'." value='{$list[description]}' /></div>";
	break;
	case 2:
...
if($met_memberlogin_code==1){   
     ...
     $metinfo.="<div class='intbtn'><input type='submit' name='Submit' value='立即匹配设计师' /></div>\n";
     $metinfo.="</form>\n";
	}
	return $metinfo;
}
